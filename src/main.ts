...
export class YTKN extends Plugin {
	...
	async onload(): Promise<void> {
		try {
			...
			notifyError('YouTube Knowledge Notes failed to load', error);
		}
	}
	...
	private getInitialGenerationOptions(): GenerationOptions {
		...
		return {
			...
			includeThumbnail: outputDefaults.includeThumbnail,
			...
			modelId: this.settings.getSelectedModel() ? buildModelId(this.settings.getSelectedModel() as ModelConfig) : undefined,
			...
			reasoningMode: resolveReasoningMode(this.settings.getSelectedModel(), outputDefaults.reasoningMode),
		...
		new GenerationOptionsModal(
			...
			(urls, options) => {
				...
				this.runQueue.enqueueBatch({
					...
					options,
					targetPolicy,
					reportPolicy: { include: options.includeRunReport ?? false, location: options.runReportLocation ?? 'first-note' },
				});
			},
			hasActiveNote,
		).open();
