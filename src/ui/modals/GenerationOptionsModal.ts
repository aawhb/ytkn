...
export class GenerationOptionsModal extends Modal {
	...
	private reasoningDropdownEl?: HTMLSelectElement;
	private reasoningDescriptionEl?: HTMLElement;
	...
	private buildInitialState(): FormState {
		...
		return {
			...
			includeThumbnail: init.includeThumbnail ?? DEFAULT_INCLUDE_THUMBNAIL,
			...
			reasoningMode: init.reasoningMode ?? DEFAULT_REASONING_MODE,
	...
	private renderQuickArea(wrap: HTMLElement): void {
		...
		const mindmapSetting = new Setting(quickGrid)
			.setName('Append mermaid mindmap')
			.addToggle((toggle) =>
				toggle
		...
		const memorableQuotesSetting = new Setting(quickGrid)
			.setName('Append memorable quotes')
			.addToggle((toggle) =>
				toggle
	...
	private renderNoteCustomizationGroup(containerEl: HTMLElement): void {
		new Setting(containerEl)
			...
			.setDesc('Embed the YouTube thumbnail near the top of the note.')
			.addToggle((toggle) =>
				toggle
					.setValue(this.state.includeThumbnail)
					.onChange((v) => (this.state.includeThumbnail = v)),
			);
		...
		new Setting(containerEl)
			.setName('Summary callout at top')
			.setDesc('Promote the AI-generated summary section into a > [!summary] callout right after the title. Visible in hover-previews.')
			.addToggle((toggle) =>
				toggle
	...
	private renderQueueAndRunReportsGroup(containerEl: HTMLElement): void {
		...
		const runReport = new Setting(containerEl)
			...
			.addDropdown((dropdown) =>
				dropdown
					.addOptions({
						'first-note': 'First generated note',
	...
	private renderAiGroup(containerEl: HTMLElement): void {
		...
		const reasoningSetting = new Setting(containerEl)
			.setName('Reasoning mode')
			.addDropdown((dropdown) => {
				this.reasoningDropdownEl = dropdown.selectEl;
				dropdown
					.addOptions({
						off: 'Off',
						on: 'On',
					...
					.setValue(this.state.reasoningMode)
					.onChange((v) => (this.state.reasoningMode = v as ReasoningMode));
			});
		this.reasoningDescriptionEl = reasoningSetting.descEl;

		new Setting(containerEl)
			.setName('Temperature')
			.setDesc('0 = deterministic. 0.3 (default) = focused. 1+ = more varied.')
			.addText((text) => {
		...
		new Setting(containerEl)
			...
			.setDesc('Increase for slow local models or reasoning-heavy runs. 300 = 5 minutes.')
			.addText((text) => {
	...
	private refreshAiVisibility(): void {
		...
		const reasoningSupport = selectedModel?.reasoningSupport ?? 'unknown';

		if (this.reasoningDropdownEl) {
			this.reasoningDropdownEl.disabled =
				reasoningSupport === 'unsupported' || !selectedModel;
			if (
				reasoningSupport === 'unsupported' &&
				this.state.reasoningMode !== 'off'
			) {
				this.state.reasoningMode = 'off';
				this.reasoningDropdownEl.value = 'off';
			}
		}
		const modelName = selectedModel?.displayName || selectedModel?.name || null;
		this.reasoningDescriptionEl?.setText(getReasoningDescription(modelName, reasoningSupport));
		this.updateControlsArea();
	}
	...
	private submit(): void {
		...
		this.onSubmit(urls, {
			...
			includeThumbnail: this.state.includeThumbnail,
			...
			controlValues: Object.keys(this.state.controlValues).length > 0
				...
				: undefined,
			reasoningMode:
				selectedModel?.reasoningSupport === 'unsupported'
					? 'off'
					: this.state.reasoningMode,
