...
export class SettingsTab extends PluginSettingTab {
	...
	private displayNoteContentSection(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(outputDefaults.includeThumbnail)
					.onChange(async (value) => {
						await this.updateOutputDefaults({
							includeThumbnail: value,
						});
					}),
			);
		...
		if (outputDefaults.includeFrontmatter) {
			...
			new Setting(containerEl)
				...
				.setDesc(
					'Space- or comma-separated list of property keys the plugin will write. Remove a key to suppress it. Allowed keys: title, aliases, source, channel, channelUrl, videoUrl, playlistUrl, videoId, playlistId, generated, videoCount.',
				)
				.addText((text) =>
					text
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Promote the AI-generated summary section into a > [!summary] callout right after the title. Visible in hover-previews.',
			)
			.addToggle((toggle) =>
				toggle
	...
	private displayAdvancedModelTuningSection(containerEl: HTMLElement): void {
		...
		const modelLabel = selectedModel?.displayName || selectedModel?.name || null;

		new Setting(containerEl).setName("Per-request defaults").setHeading();

		new Setting(containerEl)
			.setName('Reasoning / thinking by default')
			.setDesc(getReasoningDescription(modelLabel, reasoningSupport))
			.addDropdown((dropdown) => {
				dropdown
					.addOptions({
						off: 'Off',
					...
					.setValue(
						reasoningSupport === 'unsupported'
							? 'off'
							: outputDefaults.reasoningMode,
					)
					.setDisabled(reasoningSupport === 'unsupported')
					.onChange(async (value) => {
						await this.updateOutputDefaults({
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Increase for slow local models or reasoning-heavy runs. 300 = 5 minutes.',
			)
			.addText((text) => {
	...
	private displayResetSetting(containerEl: HTMLElement): void {
		new Setting(containerEl)
			...
			.setDesc(
				'Removes all providers, models, API keys, and per-feature defaults. Cannot be undone.',
			)
			.addButton((button) =>
				button
	...
	private resetSettings(): void {
		new ConfirmModal(
			...
			'This removes all providers, models, API keys, and per-feature defaults. Generated notes already in your vault are not touched.',
