...
interface AiExecutionContext {
	...
	synthesisProvider: AIModelProvider;
	promptService: PromptService;
}

function resolveReasoningMode(model: ModelConfig | null, requestedMode: ReasoningMode): ReasoningMode {
	if (model?.reasoningSupport === 'unsupported') {
		return 'off';
	}
...
export class GenerationService {
	...
	async persistBatchReport(batch: RunBatch, report: QueueBatchReport): Promise<void> {
		...
		if (batch.reportPolicy.location === 'first-note') {
	...
	private buildAiExecutionContext(effectiveOptions: GenerationOptions): AiExecutionContext | null {
		...
		const effectiveReasoningMode = resolveReasoningMode(selectedModel, effectiveOptions.reasoningMode ?? 'off');
		const { extractionProvider, synthesisProvider } = this.createProviders(
			selectedModel,
			effectiveOptions,
	...
	private validateGenerationOptions(url: string, effectiveOptions: GenerationOptions, aiContext: AiExecutionContext | null): void {
		...
		if (
			aiContext
			&& effectiveOptions.reasoningMode === 'on'
			&& aiContext.effectiveReasoningMode === 'off'
			&& aiContext.selectedModel.reasoningSupport === 'unsupported'
		) {
			new Notice(`${aiContext.selectedModel.displayName || aiContext.selectedModel.name} does not advertise reasoning support. Using reasoning off.`);
		}
	}
	...
	private createProviders(
		...
		effectiveReasoningMode: ReasoningMode,
	): { extractionProvider: AIModelProvider; synthesisProvider: AIModelProvider } {
		...
		const synthesisProvider = ProvidersFactory.createProvider(
			selectedModel,
		...
		const extractionProvider = effectiveReasoningMode === 'off'
			? synthesisProvider
			: ProvidersFactory.createProvider(
				selectedModel,
				effectiveTemperature,
				'off',
				effectiveTimeoutMs,
		...
		return { extractionProvider, synthesisProvider };
	}
	...
	private async summarizeTranscript(
		...
		const chunks = aiContext.promptService.splitTranscript(transcript, url, {
			...
			reasoningMode: aiContext.effectiveReasoningMode,
		});
		...
		if (chunks.length <= 1) {
			...
			return aiContext.synthesisProvider.summarizeVideo(aiContext.promptService.buildPrompt(transcript, url), signal);
		}
		...
		for (const [index, chunk] of chunks.entries()) {
			...
			chunkSummaries.push(
				await aiContext.extractionProvider.summarizeVideo(
		...
		return aiContext.synthesisProvider.summarizeVideo(
	...
	private async generateCombinedPlaylistNote(
		...
		let summary: string;
		try {
			summary = await aiContext.synthesisProvider.summarizeVideo(
