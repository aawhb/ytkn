...
async function generateCombinedMetadataPlaylistNote(
	...
	if (!isAppendMode) {
		await context.targets.showProgress(target, playlist.url, 'Rendering playlist metadata...', progressState);
	}
...
async function generateCombinedTranscriptPlaylistNote(
	...
	for (const [index, entry] of playlist.entries.entries()) {
		...
		try {
			if (!isAppendMode) {
				await context.targets.showProgress(target, entry.url, `Fetching transcript ${index + 1}/${playlist.entries.length}...`, progressState);
			}
	...
	if (!isAppendMode) {
		await context.targets.showProgress(target, playlist.url, 'Rendering playlist transcripts...', progressState);
	}
...
async function generateCombinedPlaylistNote(
	...
	for (const [index, entry] of playlist.entries.entries()) {
		...
		try {
			...
			const summary = await generateAiText(
				context, aiContext, transcript, entry.url, target, progressState, signal, generateSummary,
			);
	...
	if (!isAppendMode) {
		await context.targets.showProgress(target, playlist.url, finalProgress, progressState);
	}
...
export async function generatePlaylistNotes(
	...
	if (effectiveOptions.playlistMode === 'combined') {
		const { notePath, entries } = await generateCombinedPlaylistNote(
