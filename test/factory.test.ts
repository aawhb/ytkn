...
describe('ProvidersFactory', () => {
	it('creates AnthropicProvider for type anthropic', () => {
		const provider = ProvidersFactory.createProvider(makeConfig('anthropic'), 0.3, 'off', 300000);
		expect(provider).toBeInstanceOf(AnthropicProvider);
	});

	it('creates OpenAIProvider for type openai', () => {
		const provider = ProvidersFactory.createProvider(makeConfig('openai'), 0.3, 'off', 300000);
	...
	it('creates GeminiProvider for type gemini', () => {
		const provider = ProvidersFactory.createProvider(makeConfig('gemini'), 0.3, 'off', 300000);
		expect(provider).toBeInstanceOf(GeminiProvider);
	});

	it('throws for unknown provider type', () => {
		expect(() =>
			ProvidersFactory.createProvider(makeConfig('unknown-provider'), 0.3, 'off', 300000),
