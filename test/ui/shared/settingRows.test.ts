...
describe('stampSettingRowClasses', () => {
	...
	it('keeps the channel limit row inline inside the quick grid', () => {
		const host = document.createElement('div');
		const quickGrid = host.createDiv({ cls: 'ytkn-modal__quick-grid' });
		const channelLimit = new Setting(quickGrid)
			.setName('Items per type')
			.addDropdown((dropdown) => dropdown.addOption('all', 'All available'))
			.addText((text) => {
				text.inputEl.type = 'number';
			});
		channelLimit.settingEl.addClass('ytkn-channel-limit-setting');

		stampSettingRowClasses(host);

		expect(channelLimit.settingEl.classList.contains('ytkn-setting-row--select')).toBe(true);
		expect(channelLimit.settingEl.classList.contains('ytkn-setting-row--number')).toBe(true);
		expect(channelLimit.settingEl.classList.contains('ytkn-setting-row--fit-control')).toBe(true);
		expect(channelLimit.settingEl.classList.contains('ytkn-setting-row--stacked')).toBe(false);
	});
