...
describe('YouTubeService URL parsing', () => {
	it('extracts and classifies current video and playlist URL shapes', () => {
		...
		expect(YouTubeService.classifyUrls([
			'https://www.youtube.com/watch?v=abcdefghijk',
			'https://www.youtube.com/playlist?list=PL123',
			'not-a-url',
		])).toEqual(['video', 'playlist', 'invalid']);
	});
	...
	it('normalizes pasted URL lists and thumbnail URLs', () => {
		expect(YouTubeService.parseUrls(' url1, url2\nurl3 ,, ')).toEqual(['url1', 'url2', 'url3']);
...
<text start="2.0" dur="0.5">Second line</text>
	...
	it('skips unusable entries and collapses whitespace', () => {
		const xml = `<doc>
<p t="0" d="100">  </p>
<p t="100" d="100">Real\\nline
   spaced</p>
...
<p d="100">No start, ignored</p>
		...
		expect(lines).toEqual([{ text: 'Real line spaced', offset: 100 }]);
	});
