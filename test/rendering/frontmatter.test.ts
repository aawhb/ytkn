...
import { mergeFrontmatter, SACRED_FRONTMATTER_KEYS } from '../../src/rendering/frontmatter';
...
describe('mergeFrontmatter', () => {
	...
	it('covers all expected sacred keys including videoCount', () => {
		...
		expect(SACRED_FRONTMATTER_KEYS).toContain('uploadDate');
		expect(SACRED_FRONTMATTER_KEYS).toContain('videoCategory');
		...
		expect(result.merged.uploadDate).toBeUndefined();
		expect(result.merged.videoCategory).toBeUndefined();
		...
		expect(result.warnings.some((w) => w.includes('uploadDate'))).toBe(true);
		expect(result.warnings.some((w) => w.includes('videoCategory'))).toBe(true);
	});
});
