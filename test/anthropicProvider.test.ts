...
describe('AnthropicProvider', () => {
	...
	it('omits temperature when reasoningMode is on', async () => {
		mockCreate.mockResolvedValue(makeResponse('summary'));

		const provider = new AnthropicProvider('key', 'claude-3', 0.5, 'on', 300000);
	...
	it('omits deprecated temperature when reasoningMode is off', async () => {
		mockCreate.mockResolvedValue(makeResponse('summary'));

		const provider = new AnthropicProvider('key', 'claude-3', 0.5, 'off', 300000);
		await provider.summarizeVideo('prompt');
	...
	it('sends thinking config when reasoningMode is on', async () => {
		mockCreate.mockResolvedValue(makeResponse('summary'));
		...
		const [request] = mockCreate.mock.calls[0];
		expect(request.thinking).toEqual({ type: 'enabled', budget_tokens: 8192 });
	});

	it('does not send thinking config when reasoningMode is off', async () => {
		mockCreate.mockResolvedValue(makeResponse('summary'));
	...
	it('throws when response has no text blocks', async () => {
		mockCreate.mockResolvedValue({ content: [{ type: 'thinking', thinking: 'thoughts' }], stop_reason: 'end_turn' });
