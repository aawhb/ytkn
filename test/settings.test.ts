...
const baseProvider = {
	...
	apiKey: 'test-key',
	models: [{ name: 'gemini-1.5-flash', displayName: 'Gemini Flash', reasoningSupport: 'unknown' }],
};
...
describe('SettingsService', () => {
	...
	it('starts with no providers and defaults thumbnails on', async () => {
		const plugin = new FakePlugin();
		const manager = new SettingsService(plugin as any);
		await manager.loadSettings();
	...
	it('merges discovered models without duplicates, updates reasoning support, and can reset settings', async () => {
		const plugin = new FakePlugin();
		plugin.data = {
			settings: {
				providers: [
					{
						...
						models: [{ name: 'qwen3.5:4b', displayName: 'qwen3.5:4b', reasoningSupport: 'unknown' }],
					},
				],
		...
		const manager = new SettingsService(plugin as any);
		await manager.loadSettings();
		expect(manager.getOutputDefaults().transcriptMode).toBe('raw');

		const added = await manager.mergeProviderModels('Ollama', [
			{ name: 'qwen3.5:4b', displayName: 'Qwen 3.5 4B', reasoningSupport: 'supported', contextWindow: 262144 },
			{ name: 'llama3.2', displayName: 'Llama 3.2', reasoningSupport: 'unsupported', contextWindow: 131072 },
		]);

		expect(added).toBe(1);
		expect(manager.getProviders()[0].models?.map((model) => ({ name: model.name, reasoningSupport: model.reasoningSupport, contextWindow: model.contextWindow }))).toEqual([
			{ name: 'qwen3.5:4b', reasoningSupport: 'supported', contextWindow: 262144 },
			{ name: 'llama3.2', reasoningSupport: 'unsupported', contextWindow: 131072 },
		]);
	...
	it('normalizes temperature and request timeout', async () => {
		...
		expect(manager.getOutputDefaults().reasoningMode).toBe('provider-default');
		expect(plugin.saveData).toHaveBeenCalled();
	});
	...
	it('normalizes missing output defaults and preserves explicit overrides', async () => {
		...
		pluginWithSparseDefaults.data = {
			settings: {
				providers: [],
				selectedModelId: null,
				outputDefaults: { transcriptMode: 'none', includeThumbnail: true, reasoningMode: 'off', reasoningEffort: 'medium' },
				maxTokens: 1000,
				temperature: 0.2,
				requestTimeoutMs: 120000,
			},
				...
				providers: [
					{
						...
						apiKey: '',
						url: 'http://localhost:11434/v1',
						models: [{ name: 'qwen3.5:4b', displayName: 'Qwen 3.5 4B', reasoningSupport: 'unknown' }],
					},
				],
				selectedModelId: 'Ollama:qwen3.5:4b',
	...
	it('throws a descriptive error when adding a duplicate provider', async () => {
		...
		await manager.addProvider({ name: 'OpenAI', type: 'openai', apiKey: 'k', url: '', models: [] });

		await expect(
			manager.addProvider({ name: 'OpenAI', type: 'openai', apiKey: 'k', url: '', models: [] }),
	...
	it('throws when a provider name contains a colon', async () => {
		...
		await expect(
			manager.addProvider({ name: 'Open:AI', type: 'openai', apiKey: 'k', url: '', models: [] }),
	...
	it('saveProviderKey replaces the API key without re-validating other fields', async () => {
		const plugin = new FakePlugin();
		plugin.data = { settings: { providers: [baseProvider] } };
		const manager = new SettingsService(plugin as any);
		await manager.loadSettings();

		await manager.saveProviderKey('Gemini', 'new-key');
		expect(manager.getProviders()[0].apiKey).toBe('new-key');

		await expect(manager.saveProviderKey('NotThere', 'x')).rejects.toThrow(/Provider "NotThere" not found/);
	});
	...
	it('keeps the active model after renaming the owning provider', async () => {
		...
		await manager.updateProvider(
			{ name: 'GeminiPro', type: 'gemini', apiKey: 'test-key', url: undefined, models: [] },
			'Gemini',
		);
