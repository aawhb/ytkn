...
describe('versioning helpers', () => {
    it('returns null when no version is provided', () => {
        ...
        try {
            writeFileSync(join(rootDir, 'package.json'), JSON.stringify({}, null, '\t'));
            expect(resolveTargetVersion([], {} as NodeJS.ProcessEnv, rootDir)).toBeNull();
        } finally {
    ...
    it('prefers the CLI version over the environment', () => {
        ...
        expect(resolveTargetVersion([], { npm_new_version: '5.6.7' } as NodeJS.ProcessEnv)).toBe('5.6.7');
    });

    it('falls back to the current package.json version when npm has already updated package metadata', () => {
        const rootDir = mkdtempSync(join(tmpdir(), 'ytkn-versioning-'));

        try {
            writeFileSync(join(rootDir, 'package.json'), JSON.stringify({ version: '7.8.9' }, null, '\t'));

            expect(resolveTargetVersion([], {} as NodeJS.ProcessEnv, rootDir)).toBe('7.8.9');
        } finally {
            rmSync(rootDir, { recursive: true, force: true });
        }
    });

    it('updates manifest and versions files after npm has updated package metadata', () => {
        const rootDir = mkdtempSync(join(tmpdir(), 'ytkn-versioning-'));

        try {
            writeFileSync(join(rootDir, 'manifest.json'), JSON.stringify({ version: '1.0.0', minAppVersion: '1.5.0' }, null, '\t'));
            writeFileSync(join(rootDir, 'package.json'), JSON.stringify({ version: '1.2.3' }, null, '\t'));
    ...
    it('rejects version syncs when package metadata was not updated by npm first', () => {
        const rootDir = mkdtempSync(join(tmpdir(), 'ytkn-versioning-'));
        ...
        try {
            ...
            expect(() => updateVersionFiles('1.2.3', rootDir)).toThrow(/Run `npm version 1.2.3 --no-git-tag-version` first/);
        } finally {
            rmSync(rootDir, { recursive: true, force: true });
        }
    });
